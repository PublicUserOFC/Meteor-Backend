...
router.get('/fortnite/api/calendar/v1/timeline', (req: Request, res: Response) => {
    try {
        const versionInfo = functions.GetVersionInfo(req);
        
        // Map Helix version info to Neonite format
        const version = versionInfo.build.toString(); // e.g., "28.30"
        let versionGlobal = versionInfo.season; // e.g., 28
        const versionLegacy = parseInt(versionInfo.CL); // e.g., 3889387
        
        const keychainPath = path.join(__dirname, '../../../Base/responses/keychain.json');
        const keychain = JSON.parse(fs.readFileSync(keychainPath, 'utf-8'));
        
        // Config checks are skipped - all features enabled by default
        const config: any = {
            ClydeWeek1: false,
            ClydeWeek2: false,
            ClydeWeek3: false,
            ClydeWeek4: false,
            JuiceWRLDPOIStage1: false,
            JuiceWRLDPOIStage2: false,
            JuiceWRLDPOIStage3: false,
            bEnableRuinHouseBeacon: false,
            bEnableGothemCityBeacon: false,
            bEnableGreasyGroveBeacon: false,
            bEnableMoistyPalmsBeacon: false,
            bEnableFrenzyFieldsBeacon: false,
            bEnableOakBeacon: false,
            bEnableVoidBeacon: false,
            bEnableRetailRowBeacon: false,
            bEnableFloatingCubeIsland: false,
            RufusWeek2: false,
            RufusWeek3: false,
            RufusWeek4: false,
            TMNTStage1: false,
        ...
        for (const key in versionEventFlags) {
            ...
            if (versions.includes(version)) {
                versionEventFlags[key].forEach((flag: string) => {
        ...
        if (!requested) {
            ...
            console.log(`[TIMELINE] ${count} Event Flags currently active`);
            requested = true;
        }
        res.json(timeline);
    } catch (error) {
